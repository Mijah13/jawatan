        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-semibold mb-6">Tambah - Gred</h3>
                    ...
                    <!-- Add Form -->
                    <form method="POST" action="{{ route('pentadbir.gred') }}" class="mb-8">
                        @csrf
                        <div class="space-y-4">
                            <div class="bg-yellow-50 p-4 rounded-md">
                                <label for="gred" class="block text-sm font-medium text-gray-700 mb-2">Gred</label>
                                <input type="text" name="gred" id="gred" maxlength="50" required
                            ...
                            <div>
                                <label for="keutamaan"
                                    class="block text-sm font-medium text-gray-700 mb-2">Keutamaan</label>
                                <input type="number" name="keutamaan" id="keutamaan" required
                                    ...
                                    @foreach($rows as $index => $row)
                                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-yellow-50' }}">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                                <a href="{{ route('pentadbir.gred.edit', $row->id) }}"
                                                    class="text-indigo-600 hover:text-indigo-900">Edit </a>
                                                <span class="text-gray-300">|</span>
                                                <form action="{{ route('pentadbir.gred.destroy', $row->id) }}" method="POST"
