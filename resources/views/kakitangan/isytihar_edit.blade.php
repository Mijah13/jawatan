        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('harta.update', $harta->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="id_kakitangan" value="{{ $kakitangan->id }}">
                        <input type="hidden" name="id" value="{{ $harta->id }}">
                        ...
                        <table class="w-full table-auto">
                                ...
                                <td class="p-2 bg-yellow-100">
                                    <select name="jenis" class="w-full p-2 border rounded">
                                        @foreach($jenis_list as $jenis)
                                            <option value="{{ $jenis->id }}" {{ $jenis->id == $harta->jenis ? 'selected' : '' }}>
                                                {{ $jenis->jenis }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <tr>
                                ...
                                <td class="p-2">
                                    <input type="date" name="tarikhisytihar" class="p-2 border rounded" value="{{ $harta->tarikhisytihar ? $harta->tarikhisytihar->format('Y-m-d') : '' }}">
                                </td>
                            </tr>
                            <tr>
                                <td class="p-2 bg-yellow-100">Nombor Rujukan</td>
                                <td class="p-2 bg-yellow-100">
                                    <input type="text" name="no_rujukan" class="w-full p-2 border rounded" value="{{ $harta->no_rujukan }}">
                                </td>
                            </tr>
                            <tr>
                                <td class="p-2"></td>
                                <td class="p-2 flex gap-2 mt-6">
                                    ...
                                    <a href="{{ route('harta.create', ['id' => $harta->id_kakitangan]) }}"
