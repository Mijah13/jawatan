<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{ __('Laporan Kakitangan diluar CIAST') }}
        </h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <h3 class="mb-2 text-lg font-bold">Senarai Kakitangan CIAST di Luar</h3>
                            ...
                            <tbody>
                                @foreach($details as $index => $row)
                                    <tr class="{{ $index % 2 == 0 ? 'bg-yellow-50' : '' }}">
                                        ...
                                        <td class="p-2 border border-gray-300">{{ $row->jawatan }}</td>
                                        <td class="p-2 text-center border border-gray-300">{{ $row->gred }}</td>
                                        <td class="p-2 text-center border border-gray-300">{{ $row->penempatanbaru }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
