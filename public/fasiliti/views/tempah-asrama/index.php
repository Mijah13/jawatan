...
<div class="tempah-asrama-index">
    ...
    <div class="col-lg-12 bg-light-gray"> 
        ...
        <div class="card-header bg-light-gray" style="font-size:large; font-weight:bold;">
            <h1>Senarai Permohonan Tempahan -Asrama</h1>
        </div>
               ...
               'pager' => [
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'value' => function ($model) {
                            if ($model->surat_sokongan) {
                              ...
                              //  $url = '/uploads/' . $model->surat_sokongan;,     
                                    
                                    
                               //      $url = '/uploads/' . $model->surat_sokongan;,
                                    Yii::getAlias('/uploads/' . $model->surat_sokongan),
                    ...
                    [
                        ...
                        'format' => 'raw',
                        'value' => function ($model) {
                            $tarikh_masuk = $model->tarikh_masuk;
                            $tarikh_keluar = $model->tarikh_keluar;
                            // Ambil jenis_bilik yang user tempah
                            $jenisBilikUserTempah = $model->jenis_bilik;


                            // Subquery exclude bilik bertindih tempahan lain
                            $subquery = (new \yii\db\Query())
                                ->select('id_asrama')
                                ->from('tempah_asrama')
                                ->where([
                                    'and',
                                    ['is not', 'id_asrama', null],
                                    ['<', 'tarikh_masuk', $tarikh_keluar],
                                    ['>', 'tarikh_pembersihan', $tarikh_masuk],
                                ])
                                ->andWhere(['!=', 'status_tempahan_adminKemudahan', 4]);

                            // Query bilik kosong ikut tarikh
                            $asramaList = \app\models\Asrama::find()
                                ->joinWith('jenisAsrama')
                                ->where(['asrama.jenis_asrama_id' => $jenisBilikUserTempah])
                                ->andWhere(['not in', 'asrama.id', $subquery]) // ✅ filter ikut tempahan
                                ->all();
                    
                            // Mapping kelamin
                            $kelaminLabel = ['L', 'P', 'L/P'];
                    
                            // Buat array untuk dropdown
                            $asramaDropdown = ArrayHelper::map($asramaList, 'id', function ($data) use ($kelaminLabel) {
                                $kelaminText = $kelaminLabel[$data->kelamin] ?? '-'; // Safety kalau kelamin null
                                return "{$data->blok}{$data->aras}{$data->no_asrama} - {$kelaminText}";
                                // return "{$data->blok}{$data->aras}{$data->no_asrama} {$data->jenisAsrama->jenis_bilik} - {$kelaminText}";
                            });
                    
                            $selectedBilik = $model->id_asrama ?? null;

                            
                            // ✅ Tambah bilik yang dah dipilih kalau tak wujud dalam list bilik kosong
                            if ($selectedBilik && !array_key_exists($selectedBilik, $asramaDropdown)) {
                                $bilikDipilih = \app\models\Asrama::findOne($selectedBilik);
                                if ($bilikDipilih) {
                                    $kelaminText = $kelaminLabel[$bilikDipilih->kelamin] ?? '-';
                                    $label = "{$bilikDipilih->blok}{$bilikDipilih->aras}{$bilikDipilih->no_asrama} - {$kelaminText}";
                                    $asramaDropdown = [$selectedBilik => $label] + $asramaDropdown; // Add at top
                                }
                            }
                            ...
                            return Html::dropDownList("id_asrama[{$model->id}]", $selectedBilik,
                                ...
                                [
                                    ...
                                    'data-set' => !empty($model->id_asrama) ? '1' : '0'
                                ]
                            );
                        },
                        'filter' => false, // Kalau nak filter, nanti kita modify lain
                    ],
                    [
                        'label' => 'Diskaun (RM)',
                        'format' => 'raw',
                        'value' => function($model) {
                            if (in_array(Yii::$app->user->identity->role, [0, 1, 6])) {
                                $inputId = 'diskaun-input-' . $model->id;

                                return '
                                    <div style="display:flex; align-items:center; gap:5px;">
                                        <input type="number" id="'.$inputId.'" value="'.$model->diskaun.'" step="0.01" style="width:80px;" class="form-control"/>
                                        <button type="button" class="btn btn-sm btn-success" onclick="submitDiskaun('.$model->id.')">
                                            <i class="fas fa-check"></i>
                                        </button>
                                    </div>
                                ';
                            } else {
                                return $model->diskaun !== null ? Yii::$app->formatter->asDecimal($model->diskaun, 2) : '-';
                            }
                        }
                    ],                  
                    [
...
$(document).ready(function(){
    ...
    $(".bilik-pilihan").change(function(){
        const dropdown = $(this);
        const idTempahan = dropdown.data("tempahan-id");
        const idBilik = dropdown.val();
        ...
        showConfirmModal("Tetapkan bilik untuk tempahan ini?", function () {
            $.post('set-bilik?id=' + idTempahan, {
               ...
               _csrf: yii.getCsrfToken(),
                ...
                .done(function (data) {
                    if (data.success) {
                        showCustomModal("Bilik berjaya dikemaskini.", true);

                        // Kekalkan dropdown dan highlight
                ...
                .fail(function (xhr, status, error) {
                    showCustomModal("Ralat semasa mengemaskini bilik: " + error, false);
                });
...
// //update diskaun
// function submitDiskaun(id) {
//     const input = $('#diskaun-input-' + id);
//     const value = input.val();

//     if (!value || isNaN(value)) {
//         showCustomModal("Nilai diskaun tidak sah.", false);
//         return;
//     }

//     showConfirmModal("Simpan diskaun RM" + value + " untuk tempahan ini?", function () {
//         $.ajax({
//             url: 'update-diskaun?id=' + id,
//             method: 'POST',
//             data: {
//                 diskaun: value,
//                 _csrf: yii.getCsrfToken(),
...
//                 if (data.success) {
//                     showCustomModal("Diskaun berjaya dikemaskini.", true);
//                     input.css("background-color", "#d4edda");
//                 } else {
//                     showCustomModal("Gagal: " + (data.message || "Tidak diketahui."), false);
//                 }
//             },
//             error: function (xhr, status, error) {
//                 showCustomModal("Ralat: " + error, false);
//             }
//         });
//     });
// }
...
$(document).on('change', '.pelulus-pilihan', function () {
    ...
    $.ajax({
        url: '/tempah-asrama/update-pelulus',
        type: 'POST',
        data: {
            id: tempahanId, 
...
$this->registerJs(<<<JS
window.submitDiskaun = function(id) {
    const input = $('#diskaun-input-' + id);
    const value = input.val();
